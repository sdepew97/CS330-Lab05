...
if (!$ARGV[0] || !$ARGV[1] || !$ARGV[2] || !$ARGV[3] || !$ARGV[4] || !$ARGV[5] || !$ARGV[6] ) {
	...
	print "Usage:\n";
	print "$0: <number of rooms> <number of classes> <number of class times> <number of students> <contraint file> <student prefs file> <max number of sections per class> \n";
	exit 1;
}
...
my $numMinEnrollmentLimit = 18;
my $numMaxEnrollmentLimit = 75;
...
my $minclassesperstudent = 3;
my $maxclassesperstudent = 6;
...
foreach my $class ((1..$numclasses)) {
	my $newval = rand();
	my $enrollmentlimit = floor($newval * ($numMaxEnrollmentLimit - $numMinEnrollmentLimit) + $numMinEnrollmentLimit);
...
print CONSTRAINT "Teacher Preferences\n";
...
foreach my $student ((1..$numstudents)) {
	my $newval = rand();
	my $studentcap = floor($newval * ($maxclassesperstudent - $minclassesperstudent) + $minclassesperstudent);
	my @chosenclasses = ();
	for my $i ((1..$studentcap)) {
